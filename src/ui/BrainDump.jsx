...
import { fadeBackdrop, panelSurface } from './motion/tokens';

export default function BrainDump({ v }) {
  return (
    <AnimatePresence>
      {v.dumpOpen && <m.div key="brain-dump-backdrop" {...fadeBackdrop} onClick={v.toggleDump} className="fixed inset-0 z-30 bg-[rgba(43,48,52,.22)]" />}
      {v.dumpOpen && <m.div key="brain-dump-panel" {...panelSurface} className="fixed inset-0 z-[31] flex w-full flex-col bg-[#f4f6f7] p-5 pb-[max(20px,env(safe-area-inset-bottom))] sm:top-0 sm:right-0 sm:bottom-0 sm:left-auto sm:w-[380px] sm:border-l-[1.8px] sm:border-ink-line sm:p-6 sm:shadow-[-6px_0_0_rgba(58,64,69,.08)]">
