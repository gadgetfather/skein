  ...
  return (
    <>
      ...
      <m.div {...panelSurface} className="fixed inset-0 z-[33] flex w-full flex-col bg-[#f4f6f7] sm:top-0 sm:right-0 sm:bottom-0 sm:left-auto sm:w-[min(680px,52vw)] sm:border-l-[1.8px] sm:border-ink-line sm:shadow-[-8px_0_0_rgba(58,64,69,.08)]">
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-6 sm:px-[30px]">
          <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:gap-[22px]">
            <div className="relative h-[132px] w-[132px] flex-none self-center sm:self-auto">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3.5 sm:grid-cols-[1fr_150px]">
          ...
          <div className="mt-5">
            ...
            {exp.routeFrontier && (
              <div className="mb-2.5 rounded-[11px_8px_12px_9px] border-[1.5px] border-[rgba(122,154,111,.68)] bg-[rgba(122,154,111,.08)] p-3.5">
                ...
                <div className="flex items-start gap-2.5"><button onClick={exp.routeFrontier.onToggle} aria-label={`complete route move ${exp.routeFrontier.text}`} title="mark complete" className="mt-0.5 flex h-[20px] w-[20px] flex-none cursor-pointer items-center justify-center rounded-[6px] border-[1.5px] border-accent bg-paper-2 transition-colors hover:bg-[rgba(122,154,111,.12)]"></button><div><div className="text-sm font-semibold leading-[1.35] text-ink">{exp.routeFrontier.text}</div><div className="mt-1 text-[11px] leading-[1.4] text-muted-2">{exp.routeFrontier.duration&&<span className="mr-1.5 font-semibold text-accent-deep">~{exp.routeFrontier.duration} min</span>}done when: {exp.routeFrontier.doneWhen}</div></div></div>
              </div>
            )}
            {exp.steps.map(st => (
        ...
        <div className="flex flex-none flex-wrap items-center gap-2 border-t-[1.5px] border-[#dfe4e6] bg-[#eef1f2] px-4 py-3 pb-[max(12px,env(safe-area-inset-bottom))] sm:flex-nowrap sm:gap-2.5 sm:px-[30px] sm:py-3.5">
