    ...
    >
      ...
      <div className="relative mx-auto flex min-h-dvh w-[min(760px,calc(100vw-32px))] flex-col justify-center py-20 text-center">
        ...
        <div className="flex flex-wrap items-center gap-2 px-3 py-3 text-left bg-paper-2 border-[1.8px] border-ink-line rounded-[16px_13px_15px_12px] shadow-[3px_4px_0_rgba(58,64,69,.13)] sm:flex-nowrap sm:gap-3 sm:px-3.5">
          ...
          <m.button whileTap={{scale:.97}} onClick={v.weave} disabled={v.weaving} title="weave into a map" className="flex flex-1 cursor-pointer items-center justify-center gap-[7px] px-4 py-[9px] text-[15px] font-bold text-white bg-accent border-[1.6px] border-ink-line rounded-[12px_10px_12px_9px] shadow-[2px_2px_0_rgba(58,64,69,.18)] disabled:cursor-wait disabled:opacity-70 sm:flex-none">{v.weaving ? 'weaving…' : 'weave'}<span className="text-[15px]" style={v.weaving ? { animation:'micPulse 1.4s ease-in-out infinite', display:'inline-block' } : undefined}>✦</span></m.button>
        </div>
